...
<script type="text/javascript">
	...
	var editFormProduct = function(id) {
		...
		if (id == 0) {
			// ========================================
			// CREATE NEW PRODUCT - Clear all fields
			// ========================================
			<?php
			$isHasFile = false;
			...
			// ========================================
			// EDIT EXISTING PRODUCT - Load data from API
			// ========================================
			...
			var jqxhr = $.get(url, function(data) {
					...
					foreach ($fieldStructure as $key => $val) {
						...
						// SALE PRICE FIELDS (dynamic fields from m_sale_type)
						else if (stripos($key, 'sale_price_') !== false) {
							$salePriceId = str_replace('sale_price_', '', $key);
							$fieldId = $key . '_' . $formName;  // sale_price_1_form1
					?>
								// Populate <?php echo $key; ?> (sale_type_id: <?php echo $salePriceId; ?>)
								(function() {
									var fieldId = '<?php echo $fieldId; ?>';
									var saleTypeId = <?php echo $salePriceId; ?>;

									console.log('🔥 SEKAR: Populate field:', fieldId, 'for sale_type_id:', saleTypeId);
									console.log('🔥 SEKAR: Field exists:', $('#' + fieldId).length);

									var found = false;
									if (obj.sale_prices && obj.sale_prices.length > 0) {
										console.log('🔥 SEKAR: obj.sale_prices:', obj.sale_prices);
										for (var i = 0; i < obj.sale_prices.length; i++) {
											var sp = obj.sale_prices[i];
											console.log('🔥 SEKAR: Check sp:', sp);
											if (sp.sale_type_id == saleTypeId) {
												console.log('🔥 SEKAR: MATCH! Set', fieldId, '=', sp.sale_price);
												$('#' + fieldId).val(sp.sale_price);
												console.log('🔥 SEKAR: Value now:', $('#' + fieldId).val());
												found = true;
												break;
											}
										}
									} else {
										console.log('🔥 SEKAR: obj.sale_prices EMPTY or NULL!');
									}
									if (!found) {
										console.log('🔥 SEKAR: NOT FOUND, set empty');
										$('#' + fieldId).val('');
									}
								})();
						<?php
						...
						// OUTLETS FIELD (array of outlet IDs)
						else if ($key == 'outlets') {
						?>
								// Populate outlets (multiple select)
								(function() {
									var arrOutlets = [];
									if (obj.outlets && obj.outlets.length > 0) {
										for (var i = 0; i < obj.outlets.length; i++) {
											arrOutlets.push(obj.outlets[i].outlet_id);
										}
									}
									$("#outlets_<?php echo $formName; ?>").val(arrOutlets);
								})();
					<?php
						}
					...
					// ========================================
					// POPULATE DATATABLE 1 - Product Add-ons (Kit)
					// ========================================
					...
					// ========================================
					// POPULATE DATATABLE 2 - Ingredients
					// ========================================
