            ...
            foreach ($this->mongoConnectionInfos->dbInfo as $key => $dbInfo) {
                if ($key === $selectedDB) {
                    if ($this->mongoConnectionInfos->dbInfo[$key]->srv === 'mongodb') {
                        if ($this->mongoConnectionInfos->dbInfo[$key]->userName == '') {
                            $this->m = new client(
                        ...
                        $this->m = new client(
                            $this->mongoConnectionInfos->dbInfo[$key]->srv . "://{$this->mongoConnectionInfos->dbInfo[$key]->userName}:{$this->mongoConnectionInfos->dbInfo[$key]->password}@{$this->mongoConnectionInfos->dbInfo[$key]->hostname}/{$this->mongoConnectionInfos->dbInfo[$key]->db}",
                            [
                                'journal' => $this->mongoConnectionInfos->dbInfo[$key]->journal,
                                'w' => $this->mongoConnectionInfos->dbInfo[$key]->write_concerns,
                                'readConcern' => $this->mongoConnectionInfos->dbInfo[$key]->read_concern,
